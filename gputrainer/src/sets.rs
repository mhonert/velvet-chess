...
pub struct GpuDataSamples(pub Vec<DataSample>);

impl DataSamples for GpuDataSamples {
    ...
    fn add_wpov(&mut self, idx: usize, pos: u16) {
        self.0[idx].wpov_inputs.push(pos as i64);
    }

    fn add_bpov(&mut self, idx: usize, pos: u16) {
        self.0[idx].bpov_inputs.push(pos as i64);
    }

    fn finalize(&mut self, idx: usize) {
        let sample = &mut self.0[idx];
        sample.wpov_inputs.sort();
        sample.bpov_inputs.sort();
    }
}