...
use velvet::nn::{bucket_size, piece_idx, INPUTS, KING_BUCKETS, SCORE_SCALE};
use velvet::pieces::{B, Q, R};
...
pub fn read_samples<T: DataSamples>(samples: &mut T, start: usize, file_name: &str, flip_pawnless: bool, rnd: &[usize]) {
    ...
    loop {
        ...
        samples.init(idx, reader.read_f32::<LittleEndian>().unwrap(), reader.read_u8().unwrap());
        ...
        let vmirror = white_no_pawns && black_no_pawns && flip_pawnless && r & 1 == 1;

        samples.add_wpov(idx, v_mirror_if(vmirror, h_mirror_if(mirror_white_pov, white_king)) + king_offset);
        samples.add_wpov(idx, v_mirror_if(vmirror, h_mirror_if(mirror_white_pov, black_king)) + king_offset + opp_offset);

        samples.add_bpov(idx, v_mirror_if(vmirror, v_mirror_u16(h_mirror_if(mirror_black_pov, black_king))) + king_offset);
        samples.add_bpov(idx, v_mirror_u16(v_mirror_if(vmirror, h_mirror_if(mirror_black_pov, white_king))) + king_offset + opp_offset);
        ...
        for i in 1i8..=5i8 {
            if bb_map & (1 << (i - 1)) != 0 {
                ...
                for pos in BitBoard(bb) {
                    samples.add_wpov(idx, piece_idx(i) * 64
                        + v_mirror_if(vmirror, h_mirror_if(mirror_white_pov, pos as u16))
                        + white_offset);
                    samples.add_bpov(idx, piece_idx(i) * 64
