...
use velvet::bitboard::{create_blocker_permutations, gen_bishop_attacks, gen_rook_attacks, mask_without_outline};
...
fn main() {
    ...
    let cmd: Command = match args[0].as_str() {
        "packattacks" => {
            if args.len() < 4 {
        ...
        "search" => {
            ...
            let set_count = usize::from_str(&args[3]).expect("Could not parse set_count parameter");
...
fn print_usage() {
    ...
    println!(
        "  pack <rook-candidates-input-file> <bishop-candidates-input-file <packed-magics-output-file>\n   \
    ...
    magics
        ...
        .map(|s| {
            u64::from_str_radix(s.trim().trim_start_matches("0x"), 16)
                .expect(&format!("Could not parse magic number: {}", s))
        })
        .collect()
}
...
fn optimize_attacks(
    output_file: String, rook_magic_candidates: &[[u64; 64]], rook_attacks: &[BitVec],
    ...
    loop {
        ...
        while !scores.is_iteration_finished() {
            ...
            let src_attacks = if src_pos > 63 {
                &rook_attacks[src_candidate as usize]
            } else {
                &bishop_attacks[src_candidate as usize]
            };
...
fn write_magics(writer: &mut BufWriter<File>, candidate_selections: &[i32], magic_candidates: &[[u64; 64]]) {
    let magic_nums: Vec<u64> =
        candidate_selections.iter().enumerate().map(|(i, &c)| magic_candidates[c as usize][i]).collect();
...
fn find_magics(piece: PieceType, set_size: usize, magic_num_score: fn(&[u64], u32) -> i32, output_file: String) {
    ...
    for result in rx {
        ...
        for set in 0..set_size {
            let magics_str = magics[set].iter().map(|&n| format!("0x{:016x}", n)).collect::<Vec<String>>().join(", ");
        ...
        println!(
            "Scores: {}",
            best_scores.iter().map(|&s| format!("{:⋆>6}", (s / 10000))).collect::<Vec<_>>().join(", ")
        );
    }
}

fn spawn_search_threads(
    tx: &Sender<MagicResult>, set_size: usize, piece: PieceType, magic_num_score: fn(&[u64], u32) -> i32,
) {
    for pos in 0..64 {
        thread::sleep(Duration::from_millis(pos as u64 * 10));
        let tx2 = tx.clone();
        thread::spawn(move || match piece {
            PieceType::Rook => {
...
fn find_magics_for_pos(
    tx: &Sender<MagicResult>, set_size: usize, pos: i32, gen_attacks: fn(u64, i32) -> u64, shift: u32,
    magic_mask07: u64, magic_num_score: fn(&[u64], u32) -> i32,
) {
    ...
    loop {
        ...
        loop {
            ...
            for i in 0..64 {
                ...
                let is_valid =
                    validate_magic_num(&permutations, gen_attacks, &mut move_target_table, pos, magic_num, shift);
                if is_valid {
                    let gap_count = magic_num_score(&move_target_table, shift);
                    if gap_count > local_best_gap_count
                        || (gap_count == local_best_gap_count && magic_num & toggle_mask == 0)
                    {
                        best_bit = i;
...
fn validate_magic_num(
    permutations: &[u64], gen_attacks: fn(u64, i32) -> u64, move_target_table: &mut Vec<u64>, pos: i32, magic_num: u64,
    shift: u32,
) -> bool {
...
impl fmt::Debug for BitVec {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        write!(f, "BitVec[{}]", self.0.iter().map(|&n| format!("{:064b}", n)).collect::<Vec<String>>().join(", "))
    }
}

fn initialize_attacks(
    shift: usize, magic_candidate_sets: &[[u64; 64]], gen_attacks: fn(u64, i32) -> u64,
) -> Vec<BitVec> {
    let size = 1 << shift;
...
impl ScoreHistory {
    ...
    fn index(&self, prev_pos: i32, prev_candidate: i32, pos: i32, candidate: i32) -> usize {
        ((prev_pos + 1) as usize * (self.max_candidate_count + 1) + (prev_candidate + 1) as usize)
    ...
    fn choose_next_pos(&mut self) -> (i32, i32) {
        ...
        let skip_bishops = if self.finished_rooks == 0 || finished_bishops == 64 {
            true
        } else if self.finished_rooks < 64 {
            self.rnd.rand32() & 31 > 2
        } else {
            false
        };
        ...
        let avg_score =
            (candidate_scores.iter().map(|i| i.0 as i64).sum::<i64>() / candidate_scores.len() as i64) as i32;
    ...
    fn add(
        &mut self, pos: i32, candidate: i32, max_index: i32, prev_index: i32, insert_start: usize, insert_end: usize,
    ) {
    ...
    fn finish_iteration(&mut self, attacks: &BitVec) {
        ...
        for (prev, curr) in self.selection.iter().zip(self.selection.iter().skip(1)) {
            ...
            let score =
                base_score + gap_score + (curr.max_index - prev.prev_index).abs() - (curr_width + prev_width) as i32;
