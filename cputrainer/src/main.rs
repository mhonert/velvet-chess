...
const USAGE: &str = "Usage: cputrainer [train|quantize] [thread count]";
...
fn spawn_training_threads(
    ...
    hidden_gradients: &Vec<Arc<RwLock<HiddenGradients>>>, full_test_set: &Arc<CpuDataSamples>,
...
fn train(
    ...
    base_net: Arc<RwLock<Box<Network>>>, input_gradients: Arc<InputGradients>,
    hidden_gradients: Arc<RwLock<HiddenGradients>>, full_test_set: Arc<CpuDataSamples>,
    id_source: Arc<RwLock<IDSource>>,
) {
    ...
    let sets_per_batch = id_source.read().unwrap().per_batch_count();
    let batch_size = sets_per_batch * SAMPLES_PER_SET;
    let mut training_samples = CpuDataSamples(vec![DataSample::default(); batch_size]);
