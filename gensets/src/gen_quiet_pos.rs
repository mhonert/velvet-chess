...
pub trait GenQuietPos {
    ...
    fn eval_pv_end_pos(
        &mut self, rx: Option<&Receiver<Message>>, duplicate_check: &mut HashSet<u64>, pv: &[Move], ply: u16,
    ) -> Option<(String, u16, i32)>;
}
...
impl GenQuietPos for Search {
    fn is_quiet_pv(&mut self, pv: &[Move], base_piece_count: i32) -> bool {
        if let Some((m, rest_pv)) = pv.split_first() {
            ...
            let is_quiet = move_state == 0
                && m.is_quiet()
                && !self.board.is_in_check(self.board.active_player())
                && self.is_quiet_pv(rest_pv, base_piece_count);
